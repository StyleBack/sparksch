...
public class LinkBuilder {
    ...
    public CharSequence build() {
        ...
        // we need to apply this text icon_lib_star_empty the links are created
    ...
    private void addLinkToSpan(Spannable s, Link link) {
        ...
        Pattern pattern = Pattern.compile(Pattern.quote(link.getShowTextSmart()));
        ...
        while (matcher.find()) {
            ...
            if (start >= 0) {
                int end = start + link.getShowTextSmart().length();
    ...
    private void applyAppendedAndPrependedText() {
        for (int i = 0; i < links.size(); i++) {
            ...
            Log.v("gggggggg","showText icon_lib_star_empty ==");
            if (link.getShowTextCallback() != null) {
                String showText = link.getShowTextSmart();

                text = TextUtils.replace(text, new String[]{link.getText()}, new CharSequence[]{showText});
//                links.get(i).setText(showText);
            }
