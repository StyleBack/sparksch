...
import com.doschool.ahu.widget.EmojLayout;
...
public class ChatInput extends RelativeLayout implements TextWatcher,View.OnClickListener {
    ...
    private EmojLayout emoj_lay;
    private final int REQUEST_CODE_ASK_PERMISSIONS = 100;
    private int startY=0,movY,endY;
    ...
    private void initView(){
        ...
        LinearLayout bbt=findViewById(R.id.btn_bbt);
        ...
        emoj_lay=findViewById(R.id.emoj_lay);
        emoj_lay.setEditText(editText);
    }
        ...
        switch (inputMode = mode){
            case MORE:
                if (KeyboardUtils.isSoftInputVisible((Activity) getContext())){
                    ...
                    new Handler().postDelayed(() -> morePanel.setVisibility(VISIBLE),50);
                }else {
            ...
            case EMOTICON:
                if (KeyboardUtils.isSoftInputVisible((Activity) getContext())){
                    ...
                    new Handler().postDelayed(() -> {
                        if (emoj_lay.getVisibility()==GONE){
                            if (firstSm){
                                emoj_lay.showEmojLayout();
                                firstSm=false;
                            }
                            emoj_lay.setVisibility(VISIBLE);
                        }
                    },50);
                }else {
                    if (emoj_lay.getVisibility() == GONE) {
...
//            emoj_lay.showEmojLayout();
        ...
        if (id==R.id.btn_bbt){//帮帮糖
            if (activity!=null){
                new AlertDialog.Builder(getContext()).setMessage("确定要送ta一个棒棒糖吗？")
                        .setPositiveButton("是的", (dialog, id1) -> chatView.sendBbtText())
                        .setNegativeButton("不送", (dialog, id12) -> dialog.cancel()).create().show();
            }
        }
    }
