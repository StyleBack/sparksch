...
public class CollectedActivity extends BaseActivity {
    ...
    private void initC(){
        ...
        XRvUtils.initRv(crv,manager,LinearLayoutManager.VERTICAL,true,true,true);

        microBlogAdapter=new MicroBlogAdapter(this,"mic");
        crv.setAdapter(microBlogAdapter);
        crv.setLoadingListener(new XRecyclerView.LoadingListener() {
            @Override
        ...
        microBlogAdapter.setBlogFunctionListener((position, type, blogId, userId) -> {
            SlideFunPop slideFunPop=new SlideFunPop(CollectedActivity.this);
            ...
            slideFunPop.setOnFunClick(() -> {
                if (loginDao!=null){
                    if (userId ==loginDao.getObject().getUserDO().getId()){
                        ...
                        IntentUtil.toActivity(CollectedActivity.this,bundle, ReportActivity.class);
                    }
                }
            });
        });
    ...
    private void getBlogList(){
        maps.put("microblogType",String.valueOf(CodeConfig.BLOG_COLLECT));
