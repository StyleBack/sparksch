...
public class SearchUserActivity extends BaseActivity {
    ...
    private void initRV (){
        u_rv.addItemDecoration(new RvDivider(this,LinearLayoutManager.VERTICAL,R.drawable.divider_hight));
        layoutManager=new LinearLayoutManager(this);
        XRvUtils.initRv(u_rv,layoutManager,LinearLayoutManager.VERTICAL,true,true,true);

        adapter=new FansAttentAdapter(this);
        u_rv.setAdapter(adapter);

        u_rv.setLoadingListener(new XRecyclerView.LoadingListener() {
            @Override
    ...
    private void initData(){
        ...
        map.put("size","20");
        XLNetHttps.request(ApiConfig.API_SEARCH_RESULT, map, AtBean.class, new XLCallBack() {
            @Override
            ...
            public void XLFinish() {
                if (isRef){
                    u_rv.refreshComplete();
                    isRef=false;
                }

                if (isLoad){
                    u_rv.loadMoreComplete();
                    isLoad=false;
                }
            }
        });
