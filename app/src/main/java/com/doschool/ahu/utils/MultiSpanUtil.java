 ...
 * 多span样式util
 *
 * MultiSpanUtil.create("更多例子string")
 *                 .append("例子").setTextColor(Color.RED)
 *                 .append("i").setTextColor(Color.BLACK)
 *                 .into(tv);
 */
public class MultiSpanUtil {
    ...
    static int sp2px(float spValue) {
        return (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP,
                spValue, BaseApplication.getContext().getResources().getDisplayMetrics());
    }
}
