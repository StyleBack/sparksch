...
public class BottomBarTab extends FrameLayout {
    ...
    public BottomBarTab(Context context, @DrawableRes int icon, int dcon, int ucolor, int dcolor, CharSequence title) {
        this(context, null, icon,dcon,ucolor,dcolor, title);
    }

    public BottomBarTab(Context context, AttributeSet attrs, int icon, int dcon, int ucolor, int dcolor, CharSequence title) {
        this(context, attrs, 0, icon,dcon,ucolor,dcolor, title);
    }

    public BottomBarTab(Context context, AttributeSet attrs, int defStyleAttr, int icon, int dcon, int ucolor, int dcolor, CharSequence title) {
        super(context, attrs, defStyleAttr);
        init(context, icon,dcon,ucolor,dcolor, title);
    }

    private void init(Context context, int icon,int dcon,int ucolor,int dcolor, CharSequence title) {
        mContext = context;
        xIcon=icon;
        xDicon=dcon;
        xUcolor=ucolor;
        ...
        if (icon!=0 && dcon==0 && ucolor!=0){
            mIcon.setColorFilter(ContextCompat.getColor(context, ucolor));
        }
        ...
        if (ucolor!=0){
            mTvTitle.setTextColor(ContextCompat.getColor(context, ucolor));
        }
    ...
    public void setSelected(boolean selected) {
        ...
        if (selected) {
            if (xIcon!=0){
                if (xDicon==0){
                    if (xDcolor!=0){
                        mIcon.setColorFilter(ContextCompat.getColor(mContext, xDcolor));
                    }
                }else {
                    mIcon.setImageResource(xDicon);
                }
            }
            if (xDcolor!=0){
                mTvTitle.setTextColor(ContextCompat.getColor(mContext, xDcolor));
            }
        } else {
            if (xIcon!=0){
                if (xDicon==0){
                    if (xUcolor!=0){
                        mIcon.setColorFilter(ContextCompat.getColor(mContext, xUcolor));
                    }
                }else {
                    mIcon.setImageResource(xIcon);
                }
            }
            if (xUcolor!=0){
                mTvTitle.setTextColor(ContextCompat.getColor(mContext, xUcolor));
            }
        }
    }
